...
const aggregateMetrics = (readings, meals) => {
  ...
  return {
    ...
    insulinSensitivityTrend:
      avg > 180 ? "Resistance trend" : avg < 110 ? "Improving" : "Stable",
    dailyCarbLoad: Number(carbs.toFixed(1)),
    dailyFiber: Number(fiber.toFixed(1)),
  };
};

// Reference model (10k synthetic readings) for percentile/z-score style context
const buildReferenceModel = (seedCount = 10000) => {
  // Mixed populations: normal glycemic, moderate, hyperglycemic
  // Means and stds are illustrative only
  const buckets = [
  ...
  for (let i = 0; i < seedCount; i++) {
    ...
    const bucket =
      r < buckets[0].weight
        ? buckets[0]
        : r < buckets[0].weight + buckets[1].weight
        ? buckets[1]
        : buckets[2];
    const u = Math.random();
    const v = Math.random();
...
const percentileFromAvg = (avg) => {
  ...
  return Math.max(0, Math.min(100, Math.round(phi * 100)));
};

const carbTargetForRisk = (riskLevel) => {
  // Illustrative daily net carb targets based on risk group
  const targets = {
    1: { min: 180, max: 220 },
    2: { min: 140, max: 180 },
    3: { min: 110, max: 140 },
...
const generateInsights = (metrics, riskLevel) => {
  const insights = [];
  if (riskLevel >= 3) {
    insights.push("Focus on lower glycemic meals and smaller carb portions.");
  } else {
    insights.push("Maintain balanced meals with steady carb distribution.");
  }
  if (metrics.tir < 70) {
    insights.push("Increase time-in-range by adjusting meal timing and insulin.");
  }
  if (metrics.cv > 36) {
    insights.push("High variability detected. Consider consistent meal spacing.");
  }
  if (metrics.dailyCarbLoad > 220) {
    insights.push("Daily carb load is high. Add more fiber-rich vegetables.");
  }
  return insights;
};
